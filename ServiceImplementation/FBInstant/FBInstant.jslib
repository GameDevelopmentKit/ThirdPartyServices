const eventHandlerGameObjectName = "FacebookAdsWrapper";
var LibraryMyFBPlugin = {
  ...
  var requestSuccessFuncName = "RequestShowInterstitialAdSuccessed";
  var requestFailedFuncName = "RequestShowInterstitialAdFailed";
	var ad = MyAdInstance.interstitialAd;
    if (ad == null){
      console.log('AD_NOT_LOAD');
      MyGameInstance.SendMessage(eventHandlerGameObjectName, requestSuccessFuncName);
    }
    else{
        ad.showAsync().then(function(){
            MyAdInstance.interstitialAd = null;
            console.log('AD_LOAD_SUCCESSFULLY');
            MyGameInstance.SendMessage(eventHandlerGameObjectName, requestSuccessFuncName)
        }).catch(function(err){
            FBInstant.logEvent(err.code, 1, FBInstant.getEntryPointData());
            console.error('fbinstant_showInterstitialAdAsync|preloaded|show|exception: ' + JSON.stringify(err));
            MyGameInstance.SendMessage(eventHandlerGameObjectName, requestFailedFuncName, JSON.stringify(err));
  ...
  fbinstant_isRewardVideoReady: function(adId){
  ...
  fbinstant_showRewardedVideoAsync: function(){
	  var requestSuccessedFuncName = "RequestShowRewardAdSuccessed";
	  var requestFailedFuncName = "RequestShowRewardAdFailed";
	  var ad = MyAdInstance.rewardedAd;
      if (ad == null){
        console.log(preloadFuncName);
      }else{
        ad.showAsync().then(function() {
        MyAdInstance.rewardedAd = null;
			  MyGameInstance.SendMessage(eventHandlerGameObjectName, requestSuccessedFuncName);
        }).catch(function(err) {
			      FBInstant.logEvent(err.code, 1, FBInstant.getEntryPointData());
            console.error('fbinstant_showRewardedVideoAsync|preloaded|show|exception|' + JSON.stringify(err));
            MyGameInstance.SendMessage(eventHandlerGameObjectName, requestFailedFuncName, JSON.stringify(err));
            if (err.code == "ADS_NOT_LOADED"){
                MyAdInstance.rewardedAd = nul;
                console.error('ADS_NOT_LOADED');
            }
          });
      }
  },
