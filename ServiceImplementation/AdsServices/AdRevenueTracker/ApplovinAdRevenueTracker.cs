...
namespace ServiceImplementation.AdsServices.AdRevenueTracker
{
    ...
    public class ApplovinAdRevenueTracker : IAdRevenueTracker
    {
        ...
        private void OnAdRevenueClickedEvent(string arg1, MaxSdkBase.AdInfo adInfo)
        {
            var adsRevenueEvent = this.CreateAdsRevenueEvent(adInfo);
            if (adsRevenueEvent == null) return;

            this.signalBus.Fire(new AdRevenueClickedSignal(adsRevenueEvent));            
        }

        private void OnAdRevenueLoadedEvent(string arg1, MaxSdkBase.AdInfo adInfo)
        {
            var adsRevenueEvent = this.CreateAdsRevenueEvent(adInfo);
            if (adsRevenueEvent == null) return;

            this.signalBus.Fire(new AdRevenueLoadedSignal(adsRevenueEvent));
